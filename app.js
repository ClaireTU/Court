...
function triggerGuilty() {
  ...
  // Play custom audio — base64 data URL works on Safari
...
function finishRecording() {
  // Try webm first, fallback to default mime type for Safari (which prefers mp4/aac)
  const mimeType = MediaRecorder.isTypeSupported('audio/mp4') ? 'audio/mp4'
                 : MediaRecorder.isTypeSupported('audio/webm') ? 'audio/webm'
                 : '';
  const blob = new Blob(recChunks, mimeType ? { type: mimeType } : {});

  // Convert to base64 — most reliable cross-browser approach for Safari
  const reader = new FileReader();
  reader.onloadend = () => {
    customAudioURL = reader.result; // base64 data URL
...
function playRecording() {
  ...
  // base64 data URL works reliably on Safari without permission issues
  const a = new Audio(customAudioURL);
  a.play().catch(err => {
    console.warn('Play error:', err);
    alert(lang === 'zh'
      ? '播放失敗，請確認手機未靜音，並允許瀏覽器播放聲音。'
      : 'Playback failed. Please unmute your device and allow audio.');
  });
}
